...
export const AuthProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    subscriptions.push(onSnapshot(holidaysQuery, (snapshot) => {
        const holidaysList = snapshot.docs.map(doc => {
            ...
            return { ...data, id: doc.id, date: (data.date as Timestamp).toDate() } as Holiday;
        });
  ...
  const addHoliday = useCallback(async (holidayData: Omit<Holiday, 'id' | 'status'>) => {
  ...
  const calculateMonthlyPayrollDetails = useCallback((employee: User, forYear: number, forMonth: number, employeeAttendanceEvents: AttendanceEvent[], companyHolidays: Holiday[] = []): MonthlyPayrollReport => {
    ...
    const sundaysInMonth = allDaysInMonth.filter(isSunday).map(d => format(d, 'yyyy-MM-dd'));

    let salaryAfterDeductions = 0;
    let totalActualHoursWorked = 0;
    let calculatedDeductions = 0;
    const totalStandardHoursForMonth = 30 * standardDailyHours; // Monthly goal for both modes

    if (companySettings?.salaryCalculationMode === 'hourly_deduction') {
        // Hourly Deduction Mode Calculation
        const perMinuteRate = baseSalary / (totalStandardHoursForMonth * 60);

        let actualMinutesWorkedOnWorkingDays = 0;
        employeeAttendanceEvents.forEach(e => {
            const eventDate = safeParseISO(e.timestamp);
            if (eventDate && e.status === 'Checked Out' && e.totalHours) {
                const dayStr = format(eventDate, 'yyyy-MM-dd');
                // Only count hours on non-Sunday, non-holiday working days
                if (!isSunday(eventDate) && !approvedHolidayDates.has(dayStr)) {
                    actualMinutesWorkedOnWorkingDays += e.totalHours * 60;
                }
            }
        });

        // Calculate paid minutes for Sundays and Holidays
        const paidTimeOffMinutes = (sundaysInMonth.length + approvedHolidayDates.size) * standardDailyHours * 60;
        
        const totalPaidMinutes = actualMinutesWorkedOnWorkingDays + paidTimeOffMinutes;

        salaryAfterDeductions = totalPaidMinutes * perMinuteRate;
        totalActualHoursWorked = totalPaidMinutes / 60; // For display
        calculatedDeductions = baseSalary - salaryAfterDeductions;

    } else { // 'check_in_out' mode (Full day pay if checked in and out)
        const dailyRate = baseSalary / 30;
        
        // Collect unique days where employee checked out
        const checkedOutDates = new Set<string>();
        employeeAttendanceEvents
            .filter(e => e.status === 'Checked Out' && e.timestamp)
            .forEach(e => checkedOutDates.add(format(safeParseISO(e.timestamp) as Date, 'yyyy-MM-dd')));

        // Combine all paid days into a single set to avoid double-counting
        const paidDaysUnion = new Set<string>();
        checkedOutDates.forEach(date => paidDaysUnion.add(date));
        approvedHolidayDates.forEach(date => paidDaysUnion.add(date));
        sundaysInMonth.forEach(date => paidDaysUnion.add(date));
        
        salaryAfterDeductions = paidDaysUnion.size * dailyRate;
        totalActualHoursWorked = paidDaysUnion.size * standardDailyHours; // Hours paid for, not necessarily hours worked
        calculatedDeductions = baseSalary - salaryAfterDeductions;
    }
    
    const totalApprovedAdvances = employee.advances
        ?.filter(adv => adv.status === 'approved' && adv.dateProcessed && getYear(safeParseISO(adv.dateProcessed) as Date) === forYear && getMonth(safeParseISO(adv.dateProcessed) as Date) === forMonth)
        .reduce((sum, adv) => sum + adv.amount, 0) ?? 0;

    const finalNetPayable = salaryAfterDeductions - totalApprovedAdvances;
    const totalWorkingDaysInMonth = allDaysInMonth.length - sundaysInMonth.length - approvedHolidayDates.size;
    ...
    return {
        ...
        baseSalary,
        standardDailyHours,
        totalWorkingDaysInMonth: totalWorkingDaysInMonth, // This is the actual working days excluding Sundays and holidays
        totalStandardHoursForMonth,
        totalActualHoursWorked,
        totalHoursMissed,
        hourlyRate: baseSalary / totalStandardHoursForMonth, // Adjusted hourly rate for display
  ...
  const contextValue = useMemo(() => ({
    ...
    user, role, loading, companyId, companySettings, allUsers, tasks, attendanceLog, announcements, holidays,
    login, logout, addNewEmployee, addHoliday, updateCompanySettings, updateHolidayStatus, approveHolidayForPay, addTask, updateTask, addAttendanceEvent, completeCheckout, calculateMonthlyPayrollDetails, addLeaveApplication, approveLeaveApplication, rejectLeaveApplication, requestAdvance, approveAdvance, rejectAdvance
  ]);
